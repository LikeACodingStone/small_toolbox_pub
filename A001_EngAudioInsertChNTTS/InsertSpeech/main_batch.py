...
def load_config_paths():
    ...
    original_audio_path = resolve_config_path(
        os.getenv("AUDIOSOURCE_SRC_DIR", section.get("OriginalAudioPath", "../Resource"))
    )
...
def build_existing_audio_index():
    ...
    for path in AUDIO_DIR.iterdir():
        if path.is_file() and path.suffix.lower() in OUTPUT_AUDIO_SUFFIXES:
...
def find_existing_subtitle(stem):
    logging.info("Looking for subtitle for stem=%r", stem)
    ...
    for subtitle_dir in SUBTITLE_DIRS:
        ...
        for subtitle_path in candidates:
            if subtitle_path.stem == stem:
                logging.info("Matched subtitle exact: %s", subtitle_path)
                return subtitle_path

    logging.warning("No subtitle matched for stem=%r", stem)
    return None

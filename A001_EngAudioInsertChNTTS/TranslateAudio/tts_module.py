...
async def synthesize_single(idx, text, tmp_dir, semaphore):
    ...
    # edge_tts.save() opens the destination directly and does not create parents.
    # Tasks run concurrently, so each task can safely ensure the shared directory exists.
...
def ffconcat_escape(path):
    # The concat demuxer only treats backslash escapes outside single quotes.
    # A literal apostrophe inside a quoted path must close, escape, and reopen.
